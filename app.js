...
window.stockApp = function() {
    return {
        ...
        // Dynamic Sliding Catering Extension Arrays/Objects
        cateringEvents: {},
        cateringWeek: [],
        cateringForm: {
            date: '',
            items: ['', '', '', '', '', '', '', '', '', ''] // Clean array representing the 10 inputs
        },
        cateringModal: {
            show: false,
            label: '',
        ...
        async init() {
            ...
            // Listen to Catering Events dynamically in real-time
            onSnapshot(colRef('catering_events'), (snap) => {
                const fetchedEvents = {};
                snap.docs.forEach((d) => {
                    fetchedEvents[d.id] = d.data().menuItems || [];
                });
                this.cateringEvents = fetchedEvents;
                this.calculateSlidingCateringWeek();
            });
        ...
        // Helper: Builds a clean 7-day sliding window array starting exactly on local "Today"
        calculateSlidingCateringWeek() {
            const days = [];
            const today = new Date();
            today.setHours(0, 0, 0, 0); // Normalize timestamp to clean midnight boundaries
            ...
            this.cateringWeek = days;
        },

        // Verification method: Returns true if there is an active event booked for this key
        hasCateringEvent(dateKey) {
            return this.cateringEvents[dateKey] && this.cateringEvents[dateKey].length > 0;
        },

        // Form Submission logic mapping all 10 text inputs securely
        async submitCateringForm() {
            if (!this.cateringForm.date) return alert("Please select a target date.");

            // Extract items, trim whitespace, and discard entirely blank indices
            const menuArray = this.cateringForm.items
                .map(item => item.trim())
                .filter(val => val !== "");

            try {
                // Save directly to Firestore targeting Document ID as "YYYY-MM-DD"
                await setDoc(doc(dbFs, 'catering_events', this.cateringForm.date), {
                    menuItems: menuArray
                });

                alert(`Catering matrix successfully deployed for ${this.cateringForm.date}!`);
                ...
                console.error("Error updating matrix doc: ", err);
                alert("Failed to write calendar item. Check permissions.");
            }
        },

        // Trigger action when clicking a date card: Shows clean menu layout inside of a modal
        openCateringModal(day) {
            this.cateringModal.label = day.label;
            if (this.hasCateringEvent(day.dbKey)) {
                this.cateringModal.text = this.cateringEvents[day.dbKey]
                    .map((item, idx) => `${idx + 1}. ${item}`)
                    .join('\n');
            } else {
                this.cateringModal.text = "No catering functions scheduled for this day.";
            }
